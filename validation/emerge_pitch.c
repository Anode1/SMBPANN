/* emerge_pitch.c -- a two-operation PITCH/PERIODICITY task: reuse vs recombine on a fine motif + a
 * periodic spike train. Forked from emerge_twoop.c with ONLY the task changed (single far spike ->
 * periodic spike train). The dilated-block library, train_eval, and the reuse-vs-recombine analysis
 * are identical.
 *
 ...
 *
 * Predicted crossover (by analogy to emerge_twoop): uniform dilation-3 FAILS (blind to the fine motif);
 * uniform dilation-1 pays depth to span the period; a HETEROGENEOUS mix ([1,3]: one fine block, then a
 ...
 * Self-contained C99 (reuses emerge_arch's dilated-conv engine). Build: make emerge_pitch
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
    ...
    for(s=0;s<n;s++){
        ...
        if(cls==0){ plant(X[s],q,1,g_period); y[s]=1; }
            ...
            if(urand(2)==0){                      /* neg A: fine ok, train at a NON-ALIASING wrong period */
                ...
                do { per=2+(int)urand(6); }
                while(per==g_period || g_period%per==0 || per%g_period==0);   /* per in [2,7], non-aliasing */
                plant(X[s],q,1,per);
...
int main(void)
{
    ...
    int periods[8], nper=0;                       /* default {3,4,6}; override e.g. PERIODS=6 to run one period per process */
    { const char*e=getenv("PERIODS"); char buf[64]; strncpy(buf, e&&*e?e:"3,4,6", sizeof buf-1); buf[sizeof buf-1]=0;
      { char*p=strtok(buf,","); while(p&&nper<8){ int v=atoi(p); if(v>=2) periods[nper++]=v; p=strtok(NULL,","); } }
      if(nper==0){ periods[0]=3; periods[1]=4; periods[2]=6; nper=3; } }
    ...
    printf("  p  | uni-d1 (solve,E) | uni-d3 (solve,E) | REUSE best | FREE/recomb best   | verdict\n");
    for(di=0;di<nper;di++){
