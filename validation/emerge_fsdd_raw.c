/* emerge_fsdd_raw.c -- the PROVE experiment (emerge_prove.c) on the RAW FSDD waveform, SPEAKER task.
 *
 * This is a raw-waveform variant of emerge_fsdd.c.  emerge_fsdd.c fed each clip as a whole-clip
 * mean-pooled ENVELOPE (N=64 coarse bins) and asked a DIGIT-pair question.  Both choices work against the
 * emergence test: mean-pooling the whole clip destroys the waveform oscillation a local pitch/formant
 * filter would exploit, and a short window cannot possibly identify which digit was spoken.  This fork
 * fixes both: it feeds the RAW time-domain waveform (a peak-energy window, N=128 samples = 16 ms @ 8 kHz,
 * ~2 pitch periods) and switches to a SPEAKER-pair task (SPKA vs SPKB), which a compact local filter CAN
 * solve -- the proper test of "does a compact local filter emerge on real speech, GA vs random."
 *
 * Task: a BINARY SPEAKER pair (SPKA vs SPKB, from env; default george vs jackson).  The FSDD filename is
 * <digit>_<speaker>_<idx>.wav; clips are selected by the SPEAKER field (any digit), labeled SPKA->1,
 * SPKB->0.  Each clip is framed by frame_clip() -- the peak-energy RAW N-sample window, zero-mean /
 * unit-max-abs normalized (verbatim from fsdd_frame.c).  ~1000 clips (~500 per speaker) are loaded once;
 * each seed is a fresh shuffle + 700/150/150 train/val/test split plus a fresh net init, giving the
 * paired-seed variance the GA-vs-RANDOM comparison needs.
 *
 * Two search arms at MATCHED evaluation budget (unchanged from emerge_prove/emerge_fsdd):
 ...
 * compile-time default (SEEDS, GENS, POP, ...).  Data speakers: SPKA / SPKB.  Self-contained C99 + POSIX
 * dirent.  Build: cc -std=c99 -pedantic -Wall -Wextra -O2 emerge_fsdd_raw.c -lm -o emerge_fsdd_raw
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
#define NMAX   64          /* problem size N (raw peak-energy window = 64 samples = 8 ms @ 8 kHz; kept small for tractability) */
#define K      3           /* filter width used for the compactness targets */
#define HMAX   (NMAX - K + 1)
#define NOFFMAX (NMAX + HMAX)
#define NTR    700         /* train set size (capacity; runtime split may shrink if the pool is small) */
#define NVAL   150         /* validation set size (search objective) */
#define NTE    150         /* held-out test set size (reported) */
#define POOLMAX 1200       /* max clips loaded for the two speakers (~500 + ~500) */
...
static int    g_n=128, g_h=126, g_noff=254, g_off0=126, g_rlo=126, g_rhi=128;

/* ---- runtime split sizes (start at the capacities; reduced in main if the pool is smaller) ---- */
static int    g_ntr=NTR, g_nval=NVAL, g_nte=NTE;
...
#define PARAMS(X) \
  ...
  X(STR, g_spka,    "spka",    "SPKA",    "george",         "positive-class speaker (label 1)") \
  X(STR, g_spkb,    "spkb",    "SPKB",    "jackson",        "negative-class speaker (label 0)")
...
/* full loaded pool of the two speakers (loaded once; new_task reshuffles/splits it per seed) */
 ...
 * (verbatim from fsdd_frame.c -- peak-energy RAW window; this is the raw-waveform framing that replaces
 * emerge_fsdd's whole-clip mean-pool.) */
...
/* Extract the SPEAKER token from an FSDD filename "<digit>_<speaker>_<idx>.wav" into buf (the text
 * between the 1st and 2nd underscore). Returns 0 on success, -1 if the filename is malformed. */
static int clip_speaker(const char *nm, char *buf, size_t bufsz)
{
    const char *u1 = strchr(nm, '_');
    const char *u2;
    size_t len;
    if (!u1) return -1;
    u2 = strchr(u1 + 1, '_');
...
/* Walk the recordings dir; for every clip whose SPEAKER field == spa or spb (any digit), read + frame_clip
 * into POOL, with label spa->1, spb->0. Sets NPOOL. Guards against POOL overflow. */
static void fsdd_load(const char *spa, const char *spb)
{
    ...
    while ((e = readdir(d))) {
        ...
        size_t L = strlen(nm);
        char spk[128], path[1024];
        int isa, isb, len;
        if (L < 5 || strcmp(nm + L - 4, ".wav")) continue;
        if (clip_speaker(nm, spk, sizeof spk)) continue;
        isa = !strcmp(spk, spa);
        isb = !strcmp(spk, spb);
        if (!isa && !isb) continue;
        ...
        if (len < 0) continue;
        frame_clip(raw, len, POOL[NPOOL], g_n);
        POOLy[NPOOL] = isa ? 1 : 0;
        NPOOL++;
    }
    closedir(d);
...
/* new_task(seed): a deterministic reshuffle + g_ntr/g_nval/g_nte split of the loaded POOL (NOT data
 * regeneration). Each seed = a different split + net init, giving the paired-seed variance the
 * GA-vs-RANDOM comparison needs. Requires NPOOL >= g_ntr+g_nval+g_nte (ensured in main after load). */
static void new_task(uint32_t seed)
{
    ...
    for (s = 0; s < g_ntr;  s++) { int p = perm[s];                    memcpy(Xtr[s],  POOL[p], sizeof(double) * g_n); ytr[s]  = POOLy[p]; }
    for (s = 0; s < g_nval; s++) { int p = perm[g_ntr + s];            memcpy(Xval[s], POOL[p], sizeof(double) * g_n); yval[s] = POOLy[p]; }
    for (s = 0; s < g_nte;  s++) { int p = perm[g_ntr + g_nval + s];   memcpy(Xte[s],  POOL[p], sizeof(double) * g_n); yte[s]  = POOLy[p]; }
}
...
static double run_net(char off[NOFFMAX], uint32_t seed, int on_test)
{
    ...
    double bo=0, h[HMAX]; int i,j,e,s,c=0, ns=on_test?g_nte:g_nval;
    ...
    for(e=0;e<g_tepochs;e++) for(s=0;s<g_ntr;s++){
...
int main(int argc, char**argv)
{
    ...
    set_n(NMAX);              /* g_n=128: raw peak-energy window (N is fixed by framing) */
    ...
    if(g_agg){   /* read RAW lines from stdin (from parallel chunks), print the aggregated table -- pure C, no external tools */
        ...
        printf("PROVE-FSDD-RAW (aggregated): directed energy GA vs RANDOM at matched evals, grouped mutation.\n");
    ...
    /* ---- load the two speakers ONCE, then run the paired GA-vs-RANDOM comparison over seeds ---- */
    fsdd_load(g_spka, g_spkb);
    for(i=0;i<NPOOL;i++){ if(POOLy[i]) n1++; else n0++; }

    /* clamp the split to the pool: if fewer clips than the default 700/150/150, scale down proportionally */
    if(NPOOL < g_ntr+g_nval+g_nte){
        int old_tr=g_ntr, old_val=g_nval, old_te=g_nte;
        g_ntr  = NPOOL*7/10;
        g_nval = NPOOL*3/20;
        g_nte  = NPOOL - g_ntr - g_nval;
        fprintf(stderr,"warning: only %d clips for %s/%s; reducing split %d/%d/%d -> %d/%d/%d\n",
                NPOOL, g_spka, g_spkb, old_tr,old_val,old_te, g_ntr,g_nval,g_nte);
    }
    if(g_ntr<1 || g_nval<1 || g_nte<1){
        fprintf(stderr,"FSDD load: got %d clips for %s/%s -- too few for a train/val/test split\n",
                NPOOL, g_spka, g_spkb);
        return 1;
    }

    if(!g_raw){
        printf("PROVE-FSDD-RAW: real spoken-digit SPEAKER pair (SPKA=%s [label 1] vs SPKB=%s [label 0]), raw peak-energy window N=%d (%.0f ms @ 8 kHz).\n",
               g_spka, g_spkb, g_n, 1000.0*g_n/8000.0);
        ...
        printf("loaded %d clips (%d pos / %d neg); per-seed shuffle+split %d/%d/%d; %d seeds, %d gens, pop %d.\n",
               NPOOL, n1, n0, g_ntr, g_nval, g_nte, g_seeds, g_gens, g_pop);
        printf("metrics: taps->K(=%d), contig->1, test accuracy. on-rel N/A (no ground-truth filter, reported 0).\n", K);
        printf("local-filter emergence test: does a compact contiguous pitch/formant filter emerge on the RAW waveform?\n\n");
        printf("  N   arm     taps          contig        on-rel        test        | paired GA-RANDOM (mean±SEM, win/tie/loss)\n");
    }
