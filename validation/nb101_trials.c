/* nb101_trials.c -- converter: NAS-Bench-101 tfrecord -> plain text, ONE LINE PER TRIAL SET.
 *
 * WHY THIS EXISTS, separate from nb101_extract.c. That converter AVERAGES an architecture's
 * repeated training runs into one number, which is the right table for asking "which architecture
 * is best" and the wrong one for asking anything about the ESTIMATOR. NAS-Bench-101 trains every
 * architecture 3 independent times, and those 3 validation accuracies are exchangeable replicates:
 * same architecture, different initialisation and training seed. Exchangeability is exactly what
 * licenses the selection-bias measurement -- for an architecture NOT chosen using replicate i, the
 * expected accuracy on replicate i equals that on replicate j -- so the replicates must survive
 * extraction. Averaging them away destroys the only structure the measurement needs.
 *
 * Output keeps every trial: ... cnt v0 t0 v1 t1 v2 t2
 *
 * nb101_extract.c is left untouched and its table keeps reproducing byte for byte.
 *
...
#define MAXN     7
#define MAXTRIAL 8              /* NAS-Bench-101 ships 3; keep slack and report the spread */
...
typedef struct {
    ...
    signed char op[MAXN];
    double v[MAXTRIAL], t[MAXTRIAL];   /* per-trial finals, NOT averaged */
    int    cnt;
} Arch;
...
static void parse_metrics(const unsigned char *b, size_t len, Arch *a)
{
    ...
    if (bestep >= 0.0 && a->cnt < MAXTRIAL) {
        a->v[a->cnt] = bval; a->t[a->cnt] = btest; a->cnt++;
    }
}
...
int main(int argc, char **argv)
{
    ...
    fprintf(out, "# N  adjacency(N*N bits)  op0..op{N-1}  ntrial  "
                 "val0 test0 val1 test1 ...   (op: 0=in 1=c1x1 2=c3x3 3=mp 4=out)\n");
    {
        ...
        for (s = 0; s < TSIZE; s++) {
            ...
            fprintf(out, " %d", tab[s].cnt);
            { int k; for (k = 0; k < tab[s].cnt; k++)
                fprintf(out, " %.4f %.4f", 100.0*tab[s].v[k], 100.0*tab[s].t[k]); }
            fprintf(out, "\n");
            written++;
        }
        fclose(out);
