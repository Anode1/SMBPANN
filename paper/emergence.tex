...
block, then clone and translate it, Section~3.5). Chaining in the fourth, where the search must
\emph{discover} the decomposition, does not cleanly emerge (Section~3.6).}
\label{fig:main}
\end{figure}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.74\textwidth]{fig1_pruning.pdf}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.8\textwidth]{fig6_directed.pdf}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.74\textwidth]{fig3_depth.pdf}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.76\textwidth]{fig_operators.pdf}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.8\textwidth]{fig4_reuse.pdf}
...
And the advantage grows with input size: sweeping $N$ (Figure~\ref{fig:scale}, $250$ seeds), the shared arm
stays flat ($\approx 0.94$, size-independent) while the locally-connected baseline falls to chance and
bloats to hundreds of weights; the gap \emph{widens then saturates} (4--6$\times$ the SD, most growth at
$N{=}16\to32$) and survives an oracle-supervised baseline ($+0.14$ at $N{=}16$ to $+0.38$ at $N{=}128$), so
it is not a training-rule artifact.
...
redundant channel; the clean split appears only with per-operation supervision. This draws the paper's
sharpest boundary: emergent \emph{depth} matches the receptive field cleanly (Sec.~2.5), but emergent
\emph{composition} on the channel axis needs supervision or imposed structure. It also explains a chain
